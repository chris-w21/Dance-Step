...
public class BeatEditor : Editor
{
    ...
    private void OnSceneGUI()
    {
        ...
        if (beatManager.beatLines.Length != 0)
        {
            for (int c = 0; c < beatManager.beatLines.Length; c++)
            {
                ...
                if (beatManager[c].beats.Length != 0)
                {
                    ...
                    for (int i = 0; i < beatManager[c].beats.Length; i++)
                    {
                        ...
                        //if (beatManager[c, i].clip == null && clip == null)
                        //{
                          //  clip = (AudioClip)EditorGUILayout.ObjectField(clip, typeof(AudioClip), true);
                        //}
                        //else if (clip != null)
                        //{
                          //  beatManager[c].beats[i].clip = clip;
                        //}
                        //if (beatManager[c, i].clip != null)
                        //{
                        ...
                        {
                            beatManager.beatLines[c].beats[i].division = 8;
                        }
                        if (GUILayout.Button("<<", GUILayout.Width(100f)))
                        {
                            if (beatManager[c, i].division > 8)
                            {
                                beatManager.beatLines[c].beats[i].division /= 2;
